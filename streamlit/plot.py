...
df1 = df1[['item_id', 'nom_objet', 'prix', 'craft', 'focus_rentabilite', 'total_profit_non_focus']]

df['rune_last_update'] = pd.to_datetime(df['rune_last_update'])
df['Derniere update'] = ((datetime.now() - df['rune_last_update']).dt.total_seconds() / 3600).astype(int)
df.drop(columns=['rune_last_update'], inplace=True)

# Display title in Streamlit app
st.markdown("<h1 style='text-align: center; color: #FFD700; font-size: 80px;'>Richus</h1>", unsafe_allow_html=True)
st.markdown("&nbsp;")
st.markdown("&nbsp;")
st.markdown("&nbsp;")
st.markdown("<h2 style='text-align: center; color: white;'>Résumé (Hell Mina)</h2>", unsafe_allow_html=True)

col1, col2, col3, col4 = st.columns(4)
with col1:
    item_id_filter = st.selectbox("ID de l'objet", options=['Tout'] + sorted(list(df['item_id'].unique())), key='item_id_select')
...
with col3:
    ...
    update_filter_choice = st.selectbox("Derniere update de la rune", options=list(update_filter_options.keys()), key='update_filter')

if update_filter_options[update_filter_choice]:
    df = df[df['Derniere update'] <= update_filter_options[update_filter_choice]]

with col4:
    rentable_filter = st.selectbox("Seulement rentable", ['Non', 'Oui'], key='rentable_filter')
...
df_styled = df_styled.apply(highlight_renta_valeur, subset=['meilleur_renta_valeur'])
df_styled = df_styled.apply(highlight_renta_percent_high, subset=['meilleur_renta_percent'])
df_styled = df_styled.format({col: "{:.1f}" for col in df.select_dtypes(include='float').columns})
df_styled = df_styled.format({'Derniere update': lambda x: f"Il y a {x} heures"})
df['meilleur_renta_percent'] = df['meilleur_renta_percent'].astype(str) + '%'
st.dataframe(df_styled, width=1000, height=600, hide_index=True, use_container_width=True)

st.markdown("<h2 style='text-align: center; color: white;'>Prix</h2>", unsafe_allow_html=True)
st.dataframe(df1, width=1000, height=600, hide_index=True, use_container_width=True)

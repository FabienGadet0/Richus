...
def main():
    try:
        ...
        df_brisage_coeff = fetch_all_data_from_query(
            "./src/sql/last_brisage_coeff_per_item.sql"
        )
        ...
        df_item_runes_filtered["jet"] = (
            ((df_item_runes_filtered["max"] + df_item_runes_filtered["min"]) / 2)
            .round()
            .astype(int)
        )
        ...
        # Poids de brisage
        df_final["pdb"] = (
            3 * df_final["jet"] * df_final["rune_weight"] * df_final["level"] / 200 + 1
        )

        df_final["pdb"] = df_final["pdb"].apply(lambda x: x / 13.5 if x < 0 else x)

        # Coeff
        df_final["runes_qty"] = (df_final["pdb"] / df_final["rune_weight"]) * (
            df_final["coefficient"] / 100
        )

        # round
        df_final["runes_qty"] = df_final["runes_qty"].apply(lambda x: 0 if x < 0 else x)
        df_final["runes_qty"] = df_final.apply(
            lambda row: (
                row["runes_qty"] * row["rune_weight"]
                if 0 < row["rune_weight"] <= 1
                else row["runes_qty"]
            ),
            axis=1,
        )
        df_final["runes_qty"] = df_final.apply(
            lambda row: 0 if row["pdb"] < 0 else row["runes_qty"], axis=1
        )
        ...
        def calculate_focus_runes_qty(row, grouped_df):
            ...
            if current_pdb != 0:
                other_pdb = (
                    grouped_df.get_group(row["item_id"])["pdb"]
        ...
        df_final["focus_runes_qty"] = df_final.apply(
            lambda row: (
                calculate_focus_runes_qty(row, grouped_by_item_id)
                / row["rune_weight"]
                * (row["coefficient"] / 100)
                if row["rune_weight"] > 1
                else calculate_focus_runes_qty(row, grouped_by_item_id)
                * (row["coefficient"] / 100)
            ),
            axis=1,
        )
        df_final["focus_runes_qty"] = df_final.apply(
            lambda row: 0 if row["pdb"] < 0 else row["focus_runes_qty"], axis=1
        )

        df_final.drop_duplicates(inplace=True)
        for col in df_final.select_dtypes(include=["float"]).columns:
            df_final[col] = df_final[col].round(1)
        print(df_final[df_final["item_id"] == 14076])

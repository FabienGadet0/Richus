...
def parse_html_to_csv():
    ...
    df.rename(
        columns={"id": "item_id", "Nom de l'objet": "nom_de_lobjet"}, inplace=True
    )
    ...
    df["last_updated_fr"] = datetime.now(pytz.timezone("Europe/Paris")).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
...
def merge_hdv_id():
    ...
    merged_df = pd.merge(
        df1, df2, left_on="name", right_on="nom_de_lobjet", how="inner"
    )
